...
New features work offline the same way as the local-log path. A local write updates Sets, Stats, and Recap — and any screen that shows the same data — immediately, and reconciles with the server when the connection returns. If a feature cannot work that way, confirm the architecture with Chamiel before shipping an online-only design.
